...
data class Visit(val bookmarkId: String, val visitDates: List<Instant>) {
    fun inferAverageDailyVisits(): Double {
        val now = Instant.now()
        val pastVisitDates = visitDates.filter { it < now }
        val sinceDate: Instant = pastVisitDates.sorted().firstOrNull() ?: now
        ...
        return pastVisitDates.size / nDatesBetweenSinceDateAndToday.toDouble()
    }
